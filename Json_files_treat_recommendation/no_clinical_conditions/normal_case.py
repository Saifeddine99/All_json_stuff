from Json_files_treat_recommendation.no_clinical_conditions.First_time import first_time
from Json_files_treat_recommendation.no_clinical_conditions.treatment_reco_main import recommended_treatment
...
def normal_case(previous_state_,hba1c_records,symptoms,treatment_data):
    ...
    if(previous_state_=="First time"):
        ...
        if(hba1c_records[0]>target_):
            next_date="Your next check is after 3 months"
            proposed_med=recommended_treatment(treatment_data,hba1c_records[0],target_,symptoms) 
            
        else:
            next_date="Your next check is after 6 months"
            proposed_med=treatment_data["current treatment"]
